...
def _wait_for_window_server(milliseconds: int) -> None:
    """
    给窗口系统一个很短的生效时间，同时保持 Qt 事件循环继续处理。

    这里不用 time.sleep()，否则 hide/show 和 window level 变更可能还没被
    WindowServer 处理，就已经开始截图了。
    """
    if milliseconds <= 0:
        return
...
class FloatingBallWindow(QWidget):
    ...
    def __init__(self, config=None, parent=None):
        ...
        # 感知类操作（截图/桌面识别）时的临时隐藏/恢复状态
        self._capture_prepared = False
        self._capture_prepare_depth = 0
        self._chat_window_was_visible = False
    ...
    def _prepare_for_capture(self):
        ...
        # macOS 上仅 hide() 还不够。先把原生窗口层级降回普通层，再隐藏，
        # 可以显著降低 WindowServer 仍把置顶窗合进截图的概率。
        if sys.platform == "darwin":
    ...
    def _restore_after_capture(self):
        ...
        try:
            ...
            # show() 后重新应用 macOS 顶层 level，恢复截图前的展示行为。
            if sys.platform == "darwin":
                if getattr(self, "_capture_restore_ball_visible", False):
